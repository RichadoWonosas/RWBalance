...
const createForm = reactive({ name: '', secret: '', confirmation: '' })
const secret = ref('')
...
async function createNewLedger() {
  ...
  await session.create(createForm.name, createForm.secret)
...
<template>
  <main v-if="showLogin" class="login-shell" :class="{ 'login-transition-layer': sessionTransition !== 'idle', 'is-unlocking': sessionTransition === 'unlocking', 'is-locking': sessionTransition === 'locking' }">
    ...
    <section class="ledger-panel">
      ...
      <div class="panel-title"><div><span class="eyebrow">YOUR LEDGERS</span><h2>选择一个账本</h2></div><button class="primary" @click="dialog = 'create'">＋ 新建账本</button></div>
  ...
  <div v-if="session.isUnlocked" class="app-shell" :class="{ 'app-entering': sessionTransition === 'unlocking', 'app-leaving': sessionTransition === 'locking' }">
    ...
    <section class="content">
      ...
      <template v-if="page === 'accounts'">
        <section class="surface"><div class="section-title"><div><span class="eyebrow">NEW ACCOUNT</span><h3>添加账户</h3></div></div><div class="form-grid"><label>账户名称<input v-model="accountForm.name" placeholder="例如：日常银行卡" /></label><label class="checkbox"><input v-model="accountForm.pending" type="checkbox" />设为待支出账户</label><label v-for="currency in currencies" :key="currency">{{ currency }} 初始金额<input v-model="accountForm.initial[currency]" type="number" min="0" :max="currencyRules[currency].maxMinorUnits / 10 ** currencyRules[currency].fractionDigits" :step="10 ** -currencyRules[currency].fractionDigits" /></label></div><button class="primary" :disabled="session.busy" @click="createAccount">保存账户</button></section>
  ...
  <div v-if="dialog" class="modal-backdrop" @click.self="closeDialog">
    <section ref="modalElement" class="modal" role="dialog" aria-modal="true" tabindex="-1" :class="{ 'theme-modal': dialog === 'theme', 'transaction-entry-modal': dialog === 'transaction-entry' || dialog === 'transaction-correct', 'tag-resolution-modal': dialog === 'tag-delete-resolve' || dialog === 'audit-chain' }" @keydown="trapModalFocus"><button class="modal-close" aria-label="关闭对话框" @click="closeDialog">×</button>
      <Transition name="dialog-step" mode="out-in"><div :key="dialog" class="dialog-step">
      <template v-if="dialog === 'create'"><span class="eyebrow">NEW LEDGER</span><h2>创建本地加密账本</h2><label>账本名称<input v-model="createForm.name" autofocus /></label><label>访问口令<input v-model="createForm.secret" type="password" /></label><label>确认口令<input v-model="createForm.confirmation" type="password" @keyup.enter="createNewLedger" /></label><button class="primary full" :disabled="session.busy" @click="createNewLedger">{{ session.busy ? '正在加密…' : '创建并进入' }}</button></template>
      ...
      <template v-else-if="dialog === 'transaction-entry'">
        ...
        <div class="form-grid transaction-form">
          <label>类型<select v-model="draftForm.kind"><option value="expense">支出</option><option value="income">收入</option><option value="transfer">转移</option></select></label>
          <label>日期<input v-model="draftForm.bookedAt" type="date" /></label>
        ...
        <div v-if="draftForm.kind === 'expense'" class="tag-picker">
          ...
          <div class="collapse-shell tag-picker-collapse" :class="{ open: tagPickerOpen }" :aria-hidden="!tagPickerOpen" :inert="!tagPickerOpen">
