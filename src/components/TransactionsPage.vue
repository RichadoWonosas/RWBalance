...
<template>
  <section class="surface transaction-filter-panel sticky top-[.45rem] z-[12] !p-4 mobile:top-3">
    <div class="transaction-control-row grid grid-cols-[minmax(0,1fr)_auto] items-end gap-3 mobile:grid-cols-[minmax(0,1fr)_auto_auto] desktop:grid-cols-[minmax(220px,1fr)_minmax(150px,.55fr)_minmax(190px,.65fr)]">
      ...
      <button class="ghost transaction-order-button col-span-full flex min-h-11 min-w-11 items-center justify-center gap-[.65rem] px-[.7rem] mobile:col-auto desktop:min-w-0 desktop:px-4" aria-label="顺序调整向导" :disabled="!orderConflictCount" @click="emit('order')"><span class="hidden mobile:inline">顺序调整向导</span><span class="mobile:hidden">顺序</span><span v-if="orderConflictCount" class="pill">{{ orderConflictCount }}</span></button>
    </div>
