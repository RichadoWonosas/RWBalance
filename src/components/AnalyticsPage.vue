...
function moveChartTooltip(event: PointerEvent, position: ChartTooltipPosition) {
  ...
  position.top = Math.max(0, Math.min(rect.height, event.clientY - rect.top))
  position.horizontal = position.left < rect.width / 2 ? 'left' : 'right'
  position.vertical = position.top < 100 ? 'below' : 'above'
}
function updateTrendHover(event: PointerEvent) {
  trendHoverIndex.value = nearestChartIndex(event, trend.value.length)
  moveChartTooltip(event, trendTooltipPosition)
}
function updateExchangeHover(event: PointerEvent) {
...
function chartTooltipStyle(position: ChartTooltipPosition) {
  ...
  return { left: `${position.left}px`, top: `${position.top}px`, transform: `translate(${x}, ${y})` }
}
...
const donutSlices = computed(() => {
  ...
  return rows.map((row, index) => {
    ...
    return { ...row, percent: ratio * 100, path: donutPath(start, cursor), color: `hsl(${(props.primaryHue + index * 37) % 360} var(--rw-chart-segment-saturation) var(--rw-chart-segment-lightness))` }
  })
})
...
<template>
  <section class="analytics-toolbar surface grid grid-cols-1 items-stretch gap-4 mobile:grid-cols-[1fr_auto_auto] mobile:items-end"><div><span class="eyebrow">ANALYTICS RANGE</span><h3>{{ range.from }} — {{ range.to }}</h3></div><div class="range-buttons flex flex-wrap gap-[.4rem]"><button v-for="item in ([['30','近 30 天'],['90','近 90 天'],['year','今年'],['custom','自定义']] as const)" :key="item[0]" class="ghost small" :class="{ selected: preset === item[0] }" @click="preset = item[0]">{{ item[1] }}</button></div><div v-if="preset === 'custom'" class="custom-range col-span-full flex gap-[.7rem]"><label>开始<input name="custom-from" v-model="customFrom" type="date" /></label><label>结束<input name="custom-to" v-model="customTo" type="date" /></label></div><button class="primary" @click="openExport">导出统计 PNG</button></section>
  <div class="tag-stat-grid mb-4 grid grid-cols-1 gap-4 mobile:grid-cols-2"><section v-for="mode in (['primary','included'] as const)" :key="mode" class="surface !m-0"><span class="eyebrow">LAST 30 DAYS</span><h3>{{ mode === 'primary' ? '主标签支出 Top 3' : '任意包含标签 Top 3' }}</h3><div class="tag-stat-currencies mt-4 grid grid-cols-1 gap-[.8rem] compact:grid-cols-2"><div v-for="currency in currencies" :key="currency"><strong>{{ currency }}</strong><ol><li v-for="stat in ledger.tagStats[mode][currency]" :key="stat.tagId"><span>{{ statTagName(stat.tagId) }}</span><b>{{ formatMinorUnits(stat.minorUnits, currency) }}</b></li><li v-if="!ledger.tagStats[mode][currency].length">暂无支出</li></ol></div></div></section></div>
  <div class="analytics-summary grid grid-cols-1 gap-4 compact:grid-cols-2 desktop:grid-cols-4">
