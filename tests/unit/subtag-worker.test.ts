...
function occurrenceTimes(result: LedgerWorkerResult) {
  return Object.fromEntries((result.migrationInfo?.occurrenceEntries ?? []).map((entry) => [entry.id, `${entry.bookedAt}T12:00:00Z`]))
}
...
describe('Worker migration and hierarchy trust boundary', () => {
  it('previews without data access or writes; upgrades only after confirmation; reopens v3 normally', async () => {
    ...
    const opened = await request({ type: 'unlock', ledgerId: ledger.id, secret: 'test-passphrase', confirmMigration: true, migrationOccurredAt: occurrenceTimes(preview) })
    ...
    // The shipped old client's explicit version guard rejects v3, never downgrades it.
  ...
  it('keeps the original encrypted data on quota errors and retries safely', async () => {
    const { ledger, container } = await legacy()
    const preview = await request({ type: 'unlock', ledgerId: ledger.id, secret: 'test-passphrase' })
    const migrationOccurredAt = occurrenceTimes(preview)
    ...
    await expect(request({ type: 'unlock', ledgerId: ledger.id, secret: 'test-passphrase', confirmMigration: true, migrationOccurredAt })).rejects.toThrow('空间不足')
    expect(await repository.getContainer(ledger.id)).toEqual(container)
    ...
    vi.restoreAllMocks()
    expect((await request({ type: 'unlock', ledgerId: ledger.id, secret: 'test-passphrase', confirmMigration: true, migrationOccurredAt })).view).toBeDefined()
  })
  it('uses the same migration gate for import, copy and v1 recovery', async () => {
    ...
    const preview = await request(command)
    expect(preview.migrationInfo).toBeDefined()
    const migrationOccurredAt = occurrenceTimes(preview)
    expect(await repository.getContainer(ledger.id)).toBeUndefined()
    await request({ ...command, confirmMigration: true, migrationOccurredAt })
    expect((await decryptLedger((await repository.getContainer(ledger.id))!, 'test-passphrase')).schemaVersion).toBe(3)
    const copy = await request({ ...command, mode: 'copy', confirmMigration: true, migrationOccurredAt })
    ...
    const localPreview = await request({ type: 'unlock', ledgerId: local.ledger.id, secret: 'test-passphrase' })
    await request({ type: 'unlock', ledgerId: local.ledger.id, secret: 'test-passphrase', confirmMigration: true, migrationOccurredAt: occurrenceTimes(localPreview) })
    const before = await repository.getContainer(local.ledger.id)
    const recoveryPreview = await request({ type: 'restore-recovery', secret: 'test-passphrase' })
    expect(recoveryPreview.migrationInfo).toBeDefined()
    expect(await repository.getContainer(local.ledger.id)).toEqual(before)
    expect((await request({ type: 'restore-recovery', secret: 'test-passphrase', confirmMigration: true, migrationOccurredAt: occurrenceTimes(recoveryPreview) })).view?.hasMigrationBackup).toBe(true)
    expect((await decryptLedger((await repository.getContainer(local.ledger.id))!, 'test-passphrase')).schemaVersion).toBe(3)
  })
  it('rejects forged parent changes, preserves session on failure and counts ancestors once', async () => {
    ...
    result = await request({ type: 'add-transactions', drafts: [{ kind: 'expense', sourceAccountId: account.id, sourceMoney: { currency: 'CNY', minorUnits: 100 }, selectedTagIds: [child.id, sibling.id], primaryTagId: child.id, bookedAt: today, occurredAt: `${today}T12:00:00Z` }] })
  ...
  it('includes both sides of currency exchange in totals and derives hidden spending tags', async () => {
    ...
    result = await request({ type: 'add-transactions', drafts: [{
      ...
      bookedAt: new Date().toISOString().slice(0, 10), occurredAt: `${new Date().toISOString().slice(0, 10)}T12:00:00Z`,
    }] })
