...
test('creates, configures, locks and unlocks a local ledger', async ({ page }) => {
  ...
  const securitySection = page.getByRole('button', { name: /账本安全/ })
  const backupSection = page.getByRole('button', { name: /备份/ })
  await expect(securitySection).toHaveAttribute('aria-expanded', 'true')
  await expect(backupSection).toHaveAttribute('aria-expanded', 'false')
  await expect(page.getByRole('heading', { name: '加密方式', exact: true })).toBeVisible()
  await backupSection.click()
  await expect(backupSection).toHaveAttribute('aria-expanded', 'true')
  await expect(securitySection).toHaveAttribute('aria-expanded', 'false')
  await expect(page.getByRole('button', { name: '导出 .rwbl 文件', exact: true })).toBeVisible()
