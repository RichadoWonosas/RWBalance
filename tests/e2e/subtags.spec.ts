...
test('nested tag management, implicit selections, edit/correction and non-overlapping reporting', async ({ page }) => {
  ...
  await expect(page.locator('.transaction-list article')).toHaveCount(1)
  await page.locator('nav button').filter({ hasText: '标签' }).click()
  await page.getByRole('button', { name: '修改 游戏付款 的父级', exact: true }).click()
  ...
  await page.getByRole('button', { name: '保存父级', exact: true }).click()
  await expect(page.locator('.tag-management-list')).toContainText('生活 › 游戏付款 › 游戏内购买')
  await page.getByRole('button', { name: '删除标签 游戏付款', exact: true }).click()
  ...
  await expect(page.locator('.tag-management-list')).toContainText('生活 › 游戏内购买')
  await expect(page.locator('.tag-management-list')).not.toContainText('游戏付款')
  await expect(page.locator('.modal')).toHaveCount(0)
