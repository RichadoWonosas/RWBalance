...
test('nested tag management, implicit selections, edit/correction and non-overlapping reporting', async ({ page }) => {
  ...
  const hierarchyHistory = page.locator('.hierarchy-history')
  const hierarchyTrigger = hierarchyHistory.getByRole('button', { name: /层级变更记录/ })
  const hierarchyShell = hierarchyHistory.locator(':scope > .collapse-shell')
  await expect(hierarchyTrigger).toHaveAttribute('aria-expanded', 'false')
  await expect(hierarchyShell).toBeHidden()
  await hierarchyTrigger.click()
  await expect(hierarchyTrigger).toHaveAttribute('aria-expanded', 'true')
  await expect(hierarchyShell).toContainText('游戏支付')
  await hierarchyTrigger.click()
  await expect(hierarchyShell).toBeHidden()
